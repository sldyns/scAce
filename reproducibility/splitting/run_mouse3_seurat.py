...
data_mat = h5py.File('../data/Mouse3.h5')
x, y = np.array(data_mat['X']), np.array(data_mat['Y'])
data_mat.close()

y_pred_init = pd.read_csv('./pred/Seurat/Mouse3_Seurat_pred.csv', header=None).values.reshape(-1).astype('int')
...
np.savez("./results/scAce_enhance_Seurat_mouse3.npz", ARI=ari, NMI=nmi, K=K, Embedding=embedding_all,
