...
mat, obs, var, uns = read_data('../data/Mouse1.h5', sparsify=False, skip_exprs=False)
x = np.array(mat.toarray())
cell_name = np.array(obs["cell_type1"])
cell_type, y = np.unique(cell_name, return_inverse=True)

y_pred_init = pd.read_csv('./pred/Seurat/Mouse1_Seurat_pred.csv', header=None).values.reshape(-1).astype('int')
...
np.savez("./results/scAce_enhance_Seurat_mouse1.npz", ARI=ari, NMI=nmi, K=K, Embedding=embedding_all,
