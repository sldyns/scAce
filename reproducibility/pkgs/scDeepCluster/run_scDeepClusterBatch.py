...
if __name__ == "__main__":
    ...
    if args.n_clusters > 0:
        y_pred, _, _, _, _ = model.fit(X=adata.X, B=b, X_raw=adata.raw.X, size_factor=adata.obs.size_factors,
        ...
        pretrain_latent = model.encodeBatch(torch.tensor(adata.X, dtype=torch.float32),
                                            torch.tensor(b, dtype=torch.float32)).cpu().numpy()
    ...
    final_latent = model.encodeBatch(torch.tensor(adata.X, dtype=torch.float32),
                                     torch.tensor(b, dtype=torch.float32)).cpu().numpy()
